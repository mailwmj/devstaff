...
| `authorize-build` | `--quote` 指向仍适用的直接建设委托，或指向 Agent 已说明开发后果后的用户肯定答复；按完整问答判断，不按关键词 | 方案未确认；结构或视觉确认缺项；**已到 `building`/`verifying`/`delivered`/`blocked` 阶段**（须先 `unblock` 或 `reopen --scope-changed`，否则项目能靠重新授权把自己从已交付状态里拿出来）；复用其他门禁那句原话 |
...
记录请求必须带判断依据，例如用户原话、体验稿路径、检查凭据或阻塞事实。门禁工具只负责按依据持久化，不替调用 Skill 重新作出专业判断；它也不判断那句用户原话在语义上是否真等于授权——语义判断仍由 `site-brief` 和 `site-builder` 负责。两者必须读取那句话直接回应的 Agent 提问、已说明的行动后果和当前阶段，而不是建立“可以/没问题/开始”等词表。工具保证的只是"这句话被逐字记下来了，且没有被重复使用"。内容寻址凭据同样只对误改可见：拥有项目写权限的恶意 Agent 能重算整套 JSON，本地脚本不是对抗它的安全边界。
...
- **首轮就把决定和后果问清。** 越门最常见的形态不是伪造，而是"Agent 在用户表态前就往前走了"，或先只问“看着怎么样”、事后又把用户的审美答复解释成开发授权。防它靠的是对话结构：说明当前只在决定什么，以及肯定后会进入哪一步，再按紧接着的答复理解；不是快照校验，也不是要求用户背规定口令。
