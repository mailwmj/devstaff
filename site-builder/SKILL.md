...
8. **自检、冻结、独立检查。** 运行项目原生命令和最小自检，停止自有服务并 handoff 冻结。调用 `site-check`，传正式入口、DesignPacket、核心任务合同、目标视口和分发条件；多切片时历史已交付切片作为回归不变量一同复验。Checker 只读；失败后按矩阵取回 Writer，修复、重新冻结并复验。
