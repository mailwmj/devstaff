...
- 局部文案、颜色、间距：走微调快道（Fast-Path），读现有约定后直接修改并自检受影响结果；涉及结构、字段或状态机变化严禁走快道。
...
7. **连续实施。** 正式修改前取得 Writer 租约。沿用工程已有模式；需要图标时读 [组件和技术选用](references/components.md)。实现成功、失败恢复、中途退出和适用状态，不因模板或规范扩大范围。原型默认 `evolve`，只有技术形态无法承载正式要求时才 `rebuild` 并写理由。
8. **自检、冻结、独立检查。** 运行项目原生命令和最小自检，停止自有服务并通过 `site-brief/scripts/state.py handoff` 冻结。完成冻结后，**必须通过 `invoke_subagent` 派发一个独立只读子智能体（如 `TypeName="research"` 或只读模式）执行 `site-check`**，传入项目根目录与 DesignPacket；**严禁在 Writer 主会话内自行编写测试脚本并自测交付**。多切片时历史已交付切片作为回归不变量一同复验。收到子智能体返回的 `passed` 与 `check_id` 后，方可执行 `state.py deliver`；若子智能体检查失败，按失败矩阵取回 Writer 租约，修复、重新冻结并再次由独立子智能体复验。
9. **交付。** 只有当前源码上的阻断检查均通过且重要项没有 `not_run` 才能 deliver。第一句告诉用户现在能做什么，再说明入口、重开方式、已完成范围、建议体验顺序、数据位置、已检查/未检查和限制。可向用户提议生成免配置启动脚本（如 `start.bat`）或离线便携包，但必须经用户明确确认后才生成，不默认代做决定。内部矩阵术语留在文件。
