...
2. **建立检查矩阵。** 读 [验证规则](references/verification.md)，把每项能力写成“前提 → 操作 → 可观察结果”，标明是否阻断交付，再选择适用检查。Bug 必须覆盖原症状；参考还原保留参考条件。
...
5. **视觉审查。** 调用 `site-design` 的只读审查分支，传入选定方向、代表页面、关键状态和目标视口。该分支不得调用 `site-brief`；没有依据时按质量标准检查，并把方向匹配标为 `not_run`。
6. **再次打开。** 从真实交付入口重新启动或打开，核对运行说明、入口、关键数据和限制。开发服务器当次可用不等于可再次使用。
7. **报告。** 每项记录前提、操作、预期、观察、状态、是否阻断、证据与限制；按影响排序 failed 项。每项证据写清它来自哪个真实产物（截图、结果 JSON、日志）或哪次 `check.py run` 的 `check_id`，再交给 `check.py matrix` 生成矩阵凭据；阻断项只有 `artifact` 或 `command` 证据可用，写不出证据的项标成 `not_run`。回执带上 `check_id`。重要检查未执行或核心任务失败时不能称全面通过，也不写任何协作状态；本 Skill 唯一的写入位置是 `.site/checks/` 下自己的检查凭据。
