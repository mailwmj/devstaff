...
| `guided`（默认） | "要新建、整体改版或改动主流程" | 收敛核心任务 → 可见方向双选 → 确认 → 实现 → 核心验证 | 一个方向 → 确认 → 可用版本 |
...
| 信息架构双选（2 种高区分度方案） | `site-design` | 新建、整体改版、信息结构变化 | 单文件灰阶骨架预览（顶部 A/B 切换）+ 状态记录 |
| 视觉风格双选（2 种气质差异显著方案） | `site-design` | 骨架确定后 | 单文件双风格体验稿 |
| 微调收敛与资产继承 | `site-design` | 用户对 Demo 提出偏好 | Demo 的 CSS 变量 → 正式 Token；核心 HTML → 第一条纵向切片模板；偏好 → `VA-*` |
| 视觉方向：母题、视觉世界、构图命题、细节签名 | `site-design` | A2 起 | 合同的"设计推导"与"视觉方向"节 |
