...
// 规范库位于本工具同级的 assets/spec/，不依赖调用者的工作目录。
const SPEC_ROOT = join(dirname(fileURLToPath(import.meta.url)), '..', 'assets', 'spec');
...
const allFiles = files.length > 0 ? files : [
  ...readdirSync(join(SPEC_ROOT, 'styles')).filter(f => f.endsWith('-DESIGN.md')).map(f => join(SPEC_ROOT, 'styles', f)),
  ...readdirSync(join(SPEC_ROOT, 'masters')).filter(f => f.endsWith('-DESIGN.md')).map(f => join(SPEC_ROOT, 'masters', f)),
];
...
for (const filePath of allFiles) {
  ...
  const isMaster = filePath.includes('/masters/') || filePath.includes('\\masters\\');
  const fileFindings = lintFile(content, name, isMaster);
...
function lintFile(content, fileName, isMaster) {
  const results = [];
  ...
  if (yaml) {
    const knownTopKeys = ['version', 'name', 'colors', 'typography', 'rounded', 'spacing', 'components', 'method-rules'];
  ...
  // Rule 10: missing-method-rules — masters must expose structured method rules,
  // so their methodology can be checked, not just read as prose in Section 4/6.
  if (isMaster && yaml) {
    const requiredKeys = ['grid', 'alignment', 'decoration', 'max-typefaces', 'signature-move'];
    const blockMatch = yaml.match(/^method-rules:\n((?:  .+\n?)+)/m);
    if (!blockMatch) {
      results.push({ file: fileName, severity: 'warning', rule: 'missing-method-rules', message: 'No "method-rules" YAML block found — master methodology is not machine-checkable' });
    } else {
      const block = blockMatch[1];
      for (const key of requiredKeys) {
        if (!new RegExp(`^\\s{2}${key}:`, 'm').test(block)) {
          results.push({ file: fileName, severity: 'warning', rule: 'missing-method-rules', message: `"method-rules" block is missing key "${key}"` });
        }
      }
    }
  }
