...
export function parseDesignMd(filePath) {
  ...
  return {
    ...
    methodRules: parseMethodRules(content),
...
export function parseTypography(content) {
  ...
  // 大师文件的 Section 3 通常是纯文本项目符号而非加粗角色名或表格，例如
  // "- Heading: `Inter`, `Helvetica Neue`, `Arial`; weight 500-650."
  // 上面的 famRegex（要求 **加粗**）匹配不到这种格式，导致 typography.families
  // 对所有大师文件都返回空——只在 Section 3 范围内单独识别这种写法。
  const section3Prose = extractSection(content, '3');
  if (section3Prose) {
    const proseRegex = /^-\s*([A-Za-z][A-Za-z /&]*?)\s*:\s*`([^`]+)`/gm;
    while ((m = proseRegex.exec(section3Prose)) !== null) {
      addFamily(typo, familyKeys, m[1].trim(), m[2]);
    }
  }
...
// method-rules 只在设计大师文件的 YAML frontmatter 中出现，把方法论的
// 关键执行规则（网格/对齐/装饰密度/字体数量上限/标志性动作）变成可被
// lint 检查的结构化字段，而不是只留在 Section 4 的散文描述里。
export function parseMethodRules(content) {
