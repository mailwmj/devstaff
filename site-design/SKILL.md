...
| `writes` | 设计分支写页面设计合同与体验稿；只读审查分支不写项目，状态由 `site-brief` 记录 |
| `schema` | 回执 `status` 沿用下文枚举；`review_complete` 不是通过，审查发现逐项用 `passed / failed / not_run / not_applicable` |
| `handoff` | 选定方向与合同交回调用者实施；只读发现交回 Checker；业务未知由设计分支交回 `site-brief` |
| `evidence` | 方向来源、参考模式的适配依据、取舍、真实渲染页面/状态/视口；未查看项为 `not_run` |
...
3. 按 [体验稿](references/prototype.md) 制作最小可见成果；在 surface brief 逐条标记首版承诺的已演示、模拟与未覆盖条件。外观问题先结构、后风格；流程问题固定内容与业务条件，只改变需要验证的状态机制。
...
6. 新建、整体改版、核心流程或信息结构变化在视觉确认后，补全同一份 `surface-brief.md` 的 `PG-* / SC-* / RP-* / CP-* / TX-* / AS-* / VA-*`，并为每条模拟/未覆盖承诺给出正式补齐事项和交接目标；附契约的 `coverage / findings` 交给 `site-builder` 实施和 `site-check` 验收。局部修改只补受影响字段；没有项目合同的简单独立修正可直接随回执交接，不为留痕创建文件。
