...
> **对应包版本：1.2.0** ｜ 最后同步：2026-09-21 ｜ 维护：`fde-dev`
...
市场页「推荐 Prompt 模板」（`metadata.agent.quickPrompts`）。
...
| `version` | `"1.2.0"` | 市场版本号、`dist/website-developer-1.2.0.zip` | SemVer；四个随包技能的 VERSION 同值 |
...
> 支持网站搭建、参考建站、设计灵感和体验升级。不用懂技术，不管是单位介绍、产品展示、活动报名还是日常事务登记，说清用途就行：先给你两版 Demo 对比，你点头我再动手，每步看得见、测得通。
...
| :---: | :--- | :--- | :---: | :--- |
| 1 | `org-portal` | 核心业务、团队与联系入口放在一页，访客一眼看清你们做什么。 | 29 | `agent/assets/showcases/org-portal.png` |
| 2 | `service-team` | 用成立时间、服务企业数和真实项目说明实力，并给出微信咨询入口。 | 31 | `agent/assets/showcases/service-team.png` |
| 3 | `product-showcase` | 按品类陈列产品、价格和适用场景，没标价的引导访客到微信问。 | 29 | `agent/assets/showcases/product-showcase.png` |
| 4 | `brand-page` | 一个长页讲完品牌故事、产品系列与到店信息，排版跟着品牌调性走。 | 31 | `agent/assets/showcases/brand-page.png` |
...
| :---: | :--- | :--- | :--- | :--- | :--- | :--- |
| 1 | `site-build` | 网站搭建 | 从想法到能打开 | `agent/assets/home/icon-build.png` | 我想做一个网站，用途是……（比如单位介绍、产品展示、活动报名或日常登记）。你先问我几个最关键的问题。 | `site-brief` `site-design` |
| 2 | `site-reference` | 参考建站 | 发我网址或截图 | `agent/assets/home/icon-reference.png` | 我很喜欢这个网站：〈粘贴网址或截图〉，想要同样的感觉，内容换成我们自己的。 | `site-design` `site-builder` |
| 3 | `design-inspiration` | 设计灵感 | 先看两版再定方向 | `agent/assets/home/icon-inspiration.png` | 我们网站要做的事是……。先别写代码，给我两版不同气质的样子挑一挑。 | `site-design` |
| 4 | `experience-upgrade` | 体验升级 | 更好看，也更好用 | `agent/assets/home/icon-upgrade.png` | 我有一个做好的页面，看着太素、点着也别扭，想让它更好看、更好用。 | `site-design` `site-builder` |
...
| :--- | :--- | :---: | :---: | :--- | :--- |
| `site-builder` | 网站开发 | `embedded` | true | 唯一业务编排者：推进状态机、按纵向切片小步增量改代码、守住不重写防线、交付时说明验过与没验什么 | 1.2.0 |
| `site-brief` | 需求梳理 | `embedded` | false | 需求前置收敛：问清核心诉求与关键边界，用通俗选项锁定首版范围；严禁向用户提问设计与信息架构 | 1.2.0 |
| `site-design` | 网站设计与体验稿 | `embedded` | false | 骨架双选（存在真实结构分歧时）、双风格高保真体验稿、参考取证与还原度比对、中文排版与设计 Token、页面设计合同 | 1.2.0 |
| `site-check` | 网站检查与验证 | `embedded` | false | 交付前客观巡检：真实浏览器端到端只读演练、移动端横向滚动排查、出具带指纹的客观事实报告 | 1.2.0 |
...
- **构建产物**：`dist/website-developer-1.2.0/` 与 `dist/website-developer-1.2.0.zip`。zip 确定性生成（条目排序、固定时间戳与权限），无改动的重建不产生新产物。
