...
_SPEC = importlib.util.spec_from_file_location("v3_design", SCRIPT)
...
class CheckContractTests(unittest.TestCase):
    ...
    def test_valid_contract_passes_prebuild(self):
        ...
        content = (self.root / ".v3/design/surface-brief.md").read_text(encoding="utf-8")
        self.assertEqual(
...
class LintUiTests(unittest.TestCase):
    ...
    def setUp(self):
        self.temp = tempfile.TemporaryDirectory()
        self.root = Path(self.temp.name)
        (self.root / ".v3" / "design").mkdir(parents=True, exist_ok=True)
    ...
    def _write(self, contract_json=None, body=None, files=None):
        ...
        (self.root / ".v3" / "design" / "surface-brief.md").write_text(
            text, encoding="utf-8")
    ...
    def _lint(self, **kwargs):
        return design.lint_ui(self.root, ".v3/design/surface-brief.md",
                              **kwargs)
    ...
    def test_changed_from_json_no_change_is_scoped(self):
        ...
        prior = self._lint()
        prior_path = self.root / ".v3" / "prior-lint.json"
...
class LintUiGitRefTests(unittest.TestCase):
    ...
    def _lint(self, **kwargs):
        return design.lint_ui(self.root, ".v3/design/surface-brief.md", **kwargs)
