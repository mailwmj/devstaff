...
def intelligence_catalog():
    ...
    return {
        ...
        'query_contract': {
            ...
            'decision_target': '.v3/design/surface-brief.md#设计方法来源',
        },
    }
...
def research(args):
    ...
    payload = {
        ...
        'decision_record': {
            ...
            'contract_target': '.v3/design/surface-brief.md#设计方法来源',
        },
        'result': result,
    }
    design_system = result.get('design_system') if isinstance(result, dict) else None
...
# --- v3 design contract check ------------------------------------------------
# The contract is the project's copy of surface-brief.md. A fixed
# ```v3-contract``` JSON fenced block indexes the IDs defined in the body
...
def _read_contract_text(root):
    ...
    if not path.is_file():
        raise ValueError('contract not found at .v3/design/surface-brief.md')
    return path.read_text(encoding='utf-8')


def _strip_contract_block(text):
    """Body text with the v3-contract fenced block removed."""
...
def _parse_contract_block(text):
    ...
    try:
        ...
        raise ValueError(f'v3-contract block is not valid JSON: {error}') from error
...
def check_contract(root, phase):
    """Check the project's v3 design contract and return a machine-readable report.
...
LINT_IGNORE_DIRS = {'.v3', '.git', 'node_modules', '__pycache__', '.DS_Store',
...
def lint_ui(root, contract_rel, changed_from=None):
    """Statically lint the project UI against the v3 design contract.
...
def main():
    ...
    check = commands.add_parser(
        'check-contract',
        help='check the project v3 design contract and emit a machine-readable report')
    check.add_argument('--root', type=Path, required=True, help='project root containing .v3/design/surface-brief.md')
    ...
    lint = commands.add_parser(
        'lint-ui',
        help='statically lint the project UI against the v3 design contract')
