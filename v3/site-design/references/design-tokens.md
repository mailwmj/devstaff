...
先解析 `site-design-v3` 的安装目录，再使用绝对脚本路径；Windows 可按实际环境使用 `py -3`：

```sh
python3 /absolute/site-design-v3/scripts/design.py list
python3 /absolute/site-design-v3/scripts/design.py validate
python3 /absolute/site-design-v3/scripts/design.py sync-gallery   # 改过 tokens.json 后刷新 gallery 内嵌目录
python3 /absolute/site-design-v3/scripts/design.py build --recipe reading-journal --out /absolute/project/design-choice
python3 /absolute/site-design-v3/scripts/design.py build --recipe daily-workspace --palette ocean --density compact --shape crisp --out /absolute/project/alternate-choice
```

输出目录须为新目录或空目录，已有内容拒绝覆盖。输出 `tokens.css`、`selection.json` 与 `intent.md`；它们都是隔离的校准草稿，不是项目规格、设计方向或用户选稿凭证。只把最终采用的语义映射、项目理由和例外合并进 `.v3/design/surface-brief.md`；`site-builder-v3` 的实施计划引用该合同，不另行定义视觉事实。
...
制作体验稿时只把 CSS 和按需使用的 `primitives.css` 复制到隔离的体验稿目录；把选定 token 和文件路径交给 `site-builder-v3`，由它按项目结构应用到正式工程。最终运行不能依赖 Skill 安装路径。工具只生成 token，不替换现有页面、存储或业务逻辑。
