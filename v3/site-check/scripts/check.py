#!/usr/bin/env python3
"""Lightweight, deterministic check protocol for site-check-v3.

Generates a verification plan and validates check reports against the v3
protocol. It never starts a browser: it only reads the contract and source,
...
Rules encoded here:
  ...
  * A contract SHA-256 change invalidates every axis; a source SHA-256 change
    invalidates L1-L5 (the contract lives under ``.v3`` and is hashed
  ...
  * --changed-from REF accepts two kinds of input with no ambiguity: a path to
    a prior plan/report JSON (fingerprints are read from it) OR a git revision
    (branch / tag / commit) resolvable in the project's repository; the contract
    and source tree are re-fingerprinted at that revision and compared. An REF
    that is neither a readable report nor a resolvable revision is reported
    machine-readably and conservatively upgrades to guided-core — it never
    silently claims nothing changed. Git resolution is stdlib-only (subprocess),
    deterministic, and never starts a browser.
"""
...
# contract is a separate file under .v3, so L0 stays valid when only source changes.
SOURCE_INVALIDATES = ('static_build', 'core_task', 'negative_path', 'visual_desktop', 'visual_mobile', 'reopen', 'risk')

SOURCE_IGNORE = {'.v3', '.git', 'node_modules', '__pycache__', '.DS_Store'}
...
def _parse_contract_block(text: str) -> dict:
    ...
    if not match:
        raise ValueError('v3-contract block is missing')
    try:
        data = json.loads(match.group(1))
    except json.JSONDecodeError as exc:
        raise ValueError(f'v3-contract block is not valid JSON: {exc}') from exc
    if not isinstance(data, dict):
        raise ValueError('v3-contract block must be a JSON object')
    return data
...
def source_sha256(root) -> str:
    ...
    The contract lives under .v3 and is excluded, so contract and source
...
def read_mode(root) -> str:
    """Project mode from .v3/state.json, or 'guided' when there is no state."""
    path = Path(root) / STATE_REL_PATH
    if not path.is_file():
        return 'guided'
    try:
...
def plan(root, contract_rel: str, changed_from: str | None = None) -> dict:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    val_p = sub.add_parser('validate-report', help='validate a check report against the v3 protocol')
