#!/usr/bin/env python3
"""Install the four v3 skills as a sibling bundle."""
...
def load_bundle(root: Path) -> dict:
    ...
    if config.get("version") != 3 or not isinstance(skills, dict) or not skills:
        raise ValueError("invalid v3 skills.json")
...
def install(root: Path, destination: Path, replace: bool = False) -> dict:
    ...
    staging = Path(tempfile.mkdtemp(prefix="site-skills-v3-", dir=destination))
    try:
        for name, details in skills.items():
            shutil.copytree(
