...
| 任务 | 产出 | 怎么读 |
| --- | --- | --- |
| 新建、整体改版、核心流程或信息结构变化 | 有结构分歧时做 2 种信息架构（带骨架预览）➔ 2 种视觉风格（只比调性）➔ 继承为正式资产 | [序列 A](#序列-a新建或整体改版) |
...
| A3 | 按需做信息架构双选，再做视觉风格双选 | [体验稿](references/prototype.md) | 9KB |
...
| B4 | 按需做骨架双选，再做视觉风格双选 | [体验稿](references/prototype.md) | 9KB |
...
3. **信息架构双选（有分歧时）**：只有结构会改变核心任务或信息拓扑时，才用大白话提炼 2 种差别明显的架构方案（各自适合谁、第一眼看到什么、怎么操作），复制 `assets/design/preview-shell.html` 写成单文件骨架预览页。没有结构分歧时用 `state.py discover --structure single` 跳过这一步。
4. **视觉风格双选**：在已确定的骨架上给 2 种气质差别明显的方案，生成一个单文件静态预览页，顶栏照用同一条深色切换条，说明换成一两句说清两种气质差在哪。这一步守住轻量：不写业务逻辑代码，不跑单元测试、合同校验和浏览器深度断言。
