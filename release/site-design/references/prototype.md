...
- **轻量切换与 URL 驱动**：页面顶部提供轻量切换条 `[ 方案 A | 方案 B ]`，切换状态同步绑定至 URL 查询参数（如 `?structure=board` / `?structure=wizard`），刷新不丢失且支持链接直达。
...
视觉体验稿铁律：
- **单轴调性差异（Different answers, not different tints）**：在同一骨架上通过不同母题、色彩角色、字阶韵律与卡片形态拉开实质气质差异，禁止毫无解释的表层换色；
- **URL Query 变体切换**：单文件双风格通过 URL 查询参数（如 `?style=modern` / `?style=editorial`）或顶部浮动选择器进行无感切换，天然支持链接分享与再次打开；
