...
| 未决问题 | 成果 |
| --- | --- |
| 页面如何组织、什么最重要、核心路径如何展开且存在结构分歧 | 骨架体验稿（2 种高区分度结构，通俗白话对比） |
| 整体气质、排版、色彩、质感与细节调性 | 视觉体验稿（在已定骨架上默认 2 种差异显著的视觉风格） |
| 操作顺序、可达状态或业务规则 | 流程/状态体验稿 |

新建与整体改版只有在结构分歧会改变核心任务时才做骨架体验稿双选；结构确定后再做视觉体验稿双选并继承资产。没有结构分歧时直接进入视觉体验稿。
...
- **图标保持一套体系**：默认使用 Lucide；用户或现有工程明确指定其他体系时以该指定为准。同一界面不混第二套体系，不用 Emoji 或 Unicode 字符顶替按钮、导航和状态图标。图标只用来消除歧义，能删就删。

### 骨架体验稿（结构双选）

存在真实结构分歧时提供 2 种区分度显著的信息架构方案。用起手壳生成单文件骨架预览页（如 `prototypes/structure-preview.html`），提供文件链接并优先引导在 Agent 客户端自带浏览器打开。
