...
Show a version the user can actually see. At the structure and style choice stages, render each candidate in a browser yourself and, on a same-machine runtime, open it for the user instead of sending a path. Register its artifact/hash and audience with `project.py preview`. Opening is delivery, not reachability evidence: a local path, agent-accessible localhost URL or successful OS open command does not establish user reachability. A screenshot is an honest visual fallback, not an interactive experience. Do not publish a private prototype just to solve access.
