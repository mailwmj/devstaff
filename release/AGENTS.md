...
- 每轮只让用户做一个高影响决定。新建、整体改版或主流程变化，默认按“先选信息架构，再选视觉风格”两级确认推进。
- 选信息架构时给 2 种差别明显的方案。复制 `site-design` 的起手壳写成骨架预览页，色板和顶部深色切换条用它现成的，正文按项目自己搭。两个方案的配色和字号一致，颜色不参与方案差异，只用来分区块、分层级、标状态。切换条旁边一句话说清当前这版先看见什么、再怎么动手（不折叠、不做分段列举、不加动效）。把文件链接给用户，优先引导他在 Agent 客户端自带的浏览器里打开。用户选定后用 `select-structure` 记下他的原话。
...
1. **先读切片表**：每轮写代码前读 `.site/design/surface-brief.md` 的纵向切片表，找到第一条没做完的切片，标成进行中 `[-]`。不要凭印象跳切片或跨切片开发。
...
3. **有证据再打勾**：合同引用检查、typecheck/lint、受影响的单测和最短行为检查都通过后，在切片表里写一行客观的验证事实，才打 `[x]`，再进入下一条。

切片全部打勾后做一次集成构建，再交给 `site-check` 在浏览器里只读验证。第一次浏览器验证前，合同、类型、构建和受影响的测试必须都已通过。失败只修这次发现的问题，不重做整个项目。

## 交付

只说这几件事：入口在哪、完成了哪个核心任务、检查了什么、没检查什么、数据存在哪里、有哪些限制。
