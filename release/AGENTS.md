...
- 选视觉风格时，先单独问一轮参照物：有想照着做的网站或截图就发我，心里有参照的产品报个名字也行，都没有我直接给两版。问之前先交底：他自己的产品或品牌素材可以照搬，别人家的站只贴布局、密度、层级和调性，不搬 Logo、品牌色、专有字体和标志性画面。给得出参考就按 `site-design/references/reference-input.md` 做一版贴着参考的；给不出才在已定的骨架上给 2 种气质差别明显的方案，生成一个单文件双风格预览页，顶栏照用同一条深色切换条。同样把路径给他、替他打开、停下等他选。这一步只看调性，不写业务代码，不跑单元测试、合同校验和浏览器深度断言，别把时间和 Token 花在这里。
